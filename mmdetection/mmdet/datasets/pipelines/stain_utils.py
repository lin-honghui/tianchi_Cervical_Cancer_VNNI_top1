...
def standardize_brightness(I):
    ...
    return np.clip(I * 255.0 / p, 0, 255).astype(np.uint8), p
...
def normalize_rows(A):
    ...
    :return:
    """
    return A / (np.linalg.norm(A, axis=1)[:, None] + 1e-5)
...
def get_concentrations(I, stain_matrix, lamda=0.01):
    ...
    return spams.lasso(OD.T, D=stain_matrix.T, mode=2, lambda1=lamda, pos=True, numThreads=1).toarray().T
